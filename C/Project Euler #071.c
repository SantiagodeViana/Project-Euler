...
int main()
{
    int respuesta, num, dem;
    double aux, bound = 3.0/7.0, max = 0.0; //Numerador y denominador. Bound denota la fracción a la derecha, y max el valor máximo obtenido justo antes de bound
    for (dem = N; dem >= 1; dem--){
        for (num = 1; num < dem; num++){
            aux = (double)num/dem;
            if (aux < bound){
                if ((aux > max) && (isCoprime(num, dem) == 1)){ //Sólo deben considerarse fracciones reducidas
                    respuesta = num;
                    max = aux; //Se actualiza el numerador y el valor máximo obtenido
                    printf("%2d/%d Decimal: %.12f\n", num, dem, aux);
                }
            }
        }
    }
